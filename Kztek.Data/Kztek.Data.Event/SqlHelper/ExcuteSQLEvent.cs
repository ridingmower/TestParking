...
namespace Kztek.Data.Event.SqlHelper
{
    public static class ExcuteSQLEvent
    {
        //private static string connStr = "Data Source=FUTECH-PC;Initial Catalog=FU_EAZYREG;User Id=sa;Password=123456";
        private static string connStr = ConfigurationManager.ConnectionStrings["KztekEventEntities"].ConnectionString;
        /// <summary>
        ...
        {
            bool isSuccess = true;
            //FUTECHEntitiesReg db = new FUTECHEntitiesReg();
            //var objConnect = db.SystemConfigies.FirstOrDefault();
            //if (objConnect!=null)
            //{
            //    connStr = objConnect.ConnectStringGenerate;
            //}
        ...
        public static long ExecuteReturnMoney(string strSQL)
        {
            long value = 0;
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                try
                {
                    ...
                    comm.Connection = conn;
                    comm.CommandText = strSQL;
                    value = Convert.ToInt32(comm.ExecuteScalar());
                }
                catch (Exception ex)
                {
                    throw ex;
