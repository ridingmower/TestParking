...
namespace Kztek.Web.Areas.Access.Controllers
{
    public class ReportController : Controller
    {
       ...
       //test lai ////
        ...
        public ReportController(IUserService _UserService,IReportService _ReportService, ItblCardGroupService _tblCardGroupService, ItblSystemConfigService _tblSystemConfigService, ItblCustomerGroupService _tblCustomerGroupService, ItblCustomerService _tblCustomerService, ItblAccessControllerService _tblAccessControllerService, ItblAccessDoorService _tblAccessDoorService, ItblCardService _tblCardService)
        {
        ...
        [CheckSessionLogin]
        [CheckAuthorize]
        public ActionResult ReportEvent(string KeyWord = "",string status = "",string controllerid = "", string cardgroup = "", string fromdate = "", string todate = "", int page = 1, string chkExport = "0", string datefrompicker = "" , string typeEvent ="")
        {
            var totalItem = 0;
            var pageSize = 20;
            ...
            {
                var obj = _tblCustomerService.GetByCode(KeyWord);
                if (obj != null)
                {
                    
                    var objCard = _tblCardService.GetByCustomerId(obj.CustomerID.ToString());
                    foreach (var item in objCard)
                    {
                        CardNumbers += item.CardNumber + ",";
                    }
                }
            }
            ...
            {
                ////Query lấy dữ liệu
                var listExcel = _ReportService.GetReportEventExcel(KeyWord, status, controllerid, fromdate, todate, cardgroup ,typeEvent , CardNumbers);
                ...
                if(listExcel != null && listExcel.Rows.Count > 0)
                {
                    foreach(DataRow dr in listExcel.Rows)
                    {
                        ...
                        dr["CustomerCode"] = customer != null ? customer.CustomerCode : "";
            ...
            #region Giao diện
            var list = _ReportService.GetReportEvent(KeyWord, status, controllerid,fromdate,todate,page,pageSize,ref totalItem,cardgroup , typeEvent , CardNumbers);
            ...
            ViewBag.TypeEv = typeEvent;
            ViewBag.TypeEvCheck = FunctionHelper.CheckTypeEvent();
        ...
        private void ReportEventFormatCell(DataTable dtData, string filename = "", string sheetname = "", string comments = "", string titleSheet = "", string titleTime = "")
        {
            ...
            listColumn.Add(new SelectListModel { ItemText = "Sự kiện", ItemValue = "EventType" });
        ...
        #region Báo cáo nạp hủy thẻ

        public ActionResult ReportAccessUploadProcess(string controllerid = "" , string enventype = "",string actions = "", string key = "", string chkExport = "0", string fromdate = "", string todate = "", int page = 1)
       {
            var totalItem = 0;
            var pageSize = 10;
            var user = GetCurrentUser.GetUser();

            ViewBag.Controllers = _tblAccessControllerService.GetAllActive().ToList();
            ViewBag.ControllerId = controllerid;

            ViewBag.Actions = FunctionHelper.ActionUploadProcess();
            ViewBag.actionid = actions;

            ViewBag.EvenTypes = FunctionHelper.EventType();
            ViewBag.eventype = enventype;
            var strCG = new List<string>();
            ...
            {
                //Query lấy dữ liệu
                var listExcel = _ReportService.GetReportAccessUploadProcessExcel_Access(controllerid, enventype, actions, key, fromdate, todate);


                //Xuất file theo format
                ReportAccessUploadProcessExcel_AccessFormatCell(listExcel, "Báo cáo nạp hủy thẻ", "Sheet1", "", "Báo cáo nạp hủy thẻ", datefrompicker);

                return RedirectToAction("ReportAccessUploadProcess");
            }
            #endregion

            #region Giao diện
            var list = _ReportService.GetReportAccessUploadProcess(controllerid, enventype, actions,key, page, pageSize, fromdate,todate ,ref totalItem);

            var gridModel = PageModelCustom<ReporttblAccessUploadProcess_Custom>.GetPage(list, page, pageSize, totalItem);
        ...
        private void ReportAccessUploadProcessExcel_AccessFormatCell(List<ReporttblAccessUploadProcess_Excel> listData, string filename = "", string sheetname = "", string comments = "", string titleSheet = "", string titleTime = "")
        {
            //Nội dung đầu trang
            var user = GetCurrentUser.GetUser();
            ...
            listColumn.Add(new SelectListModel { ItemText = "Hình thức", ItemValue = "Hình thức" });
            listColumn.Add(new SelectListModel { ItemText = "Loại dữ liệu", ItemValue = "Loại dữ liệu" });
            listColumn.Add(new SelectListModel { ItemText = "Mã thẻ", ItemValue = "Mã thẻ" });
            ...
            //Chuyển dữ liệu về datatable
             DataTable dtData = listData.ToDataTableNullable();
        ...
        #region Báo cáo chi tiết thông tin thẻ trong bdk

        public ActionResult ReportDetailCardInController(string controllerid = "",  string key = "", string chkExport = "0", string fromdate = "", string todate = "", int page = 1)
        {
            var totalItem = 0;
            var pageSize = 20;
            var user = GetCurrentUser.GetUser();
            ...
            {
                //Query lấy dữ liệu
                var listExcel = _ReportService.GetReportDetailCardInControllerExcel_Access(controllerid, key, fromdate, todate);


                //Xuất file theo format
                ReportDetailCardInControllerExcel_Access(listExcel, "Báo cáo chi tiết thông tin thẻ BĐK", "Sheet1", "", "Báo cáo chi tiết thông tin thẻ BĐK", datefrompicker);

                return RedirectToAction("ReportDetailCardInController");
            }
            #endregion

            #region Giao diện
            var list = _ReportService.GetReportDetailCardInController(controllerid, key, page, pageSize, fromdate, todate, ref totalItem);

            var gridModel = PageModelCustom<tblAccessControllerMemory_Custom>.GetPage(list, page, pageSize, totalItem);

            ViewBag.Card_Customer = _tblCustomerService.GetCustomerJoinCard();
            ViewBag.KeyWord = key;
            ViewBag.CustomerGroups = GetMenuList();
            ViewBag.fromdateValue = fromdate;
            ViewBag.todateValue = string.IsNullOrWhiteSpace(todate) ? DateTime.Now.ToString("đd/MM/yyyy 23:59") : todate;
        ...
        private void ReportDetailCardInControllerExcel_Access(List<tblAccessControllerMemory_Excel> listData, string filename = "", string sheetname = "", string comments = "", string titleSheet = "", string titleTime = "")
        {
            //Nội dung đầu trang
            var user = GetCurrentUser.GetUser();
        ...
        #region Báo cáo sự kiện mở cửa bằng nút nhấn

        public ActionResult ReportExitButtonEvent(string controllerid = "", string chkExport = "0", string fromdate = "", string todate = "", int page = 1)
        {
            var totalItem = 0;
            var pageSize = 20;
            var user = GetCurrentUser.GetUser();
            ...
            {
                //Query lấy dữ liệu
                var listExcel = _ReportService.GetReportExitButtonEventExcel_Access(controllerid, fromdate, todate);


                //Xuất file theo format
                ReportExitButtonEventExcel_Access(listExcel, "Báo cáo sự kiện mở cửa bằng nút nhấn", "Sheet1", "", "Báo cáo sự kiện mở cửa bằng nút nhấn", datefrompicker);
            ...
            #region Giao diện
            var list = _ReportService.GetReportExitButtonEvent(controllerid,  page, pageSize, fromdate, todate, ref totalItem);

            var gridModel = PageModelCustom<tblExitButtonEvent_Custom>.GetPage(list, page, pageSize, totalItem);
        ...
        private void ReportExitButtonEventExcel_Access(List<tblExitButtonEvent_Excel> listData, string filename = "", string sheetname = "", string comments = "", string titleSheet = "", string titleTime = "")
        {
            //Nội dung đầu trang
            var user = GetCurrentUser.GetUser();
