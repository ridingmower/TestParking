...
namespace Kztek.Web.Areas.Access.Controllers
{
    public class tblCardController : Controller
    {
        ...
        public tblCardController(ItblCardService _tblCardService, ItblCardGroupService _tblCardGroupService, ItblCustomerService _tblCustomerService, ItblCustomerGroupService _tblCustomerGroupService, ItblSystemConfigService _tblSystemConfigService, ItblCardEventService _tblCardEventService, ItblAccessLevelService _tblAccessLevelService, ItblAccessLevelDetailService _tblAccessLevelDetailService, ItblAccessUploadDetailService _tblAccessUploadDetailService, ItblAccessControllerService _tblAccessControllerService)
        {
        ...
        [CheckSessionLogin]
        [CheckAuthorize]
        public ActionResult Index(string key, string cardgroups, string customerid, string customergroups, string active, string fromdate, string todate, string isCheckByTime = "0", int page = 1, string chkExport = "0", string selectedId = "", string levels = "", bool desc = true, string columnQuery = "ImportDate", string cardtype = "")
        {
            var str = GetListChild("", customergroups);

            if (chkExport == "1")
            {
                var listExcel = _tblCardService.GetAccessExcelCardByFirstTSQL(key, cardgroups, customerid, str, fromdate, todate, desc, columnQuery, isCheckByTime, "", active, cardtype);
            ...
            var list = _tblCardService.GetAllPagingByFirstTSQL(key, cardgroups, customerid, str, fromdate, todate,desc, columnQuery, ref total, isCheckByTime, page, pageSize, levels, active, cardtype);
            ...
            ViewBag.lcardTypes = FunctionHelper.CardChipTypes();
        ...
        private void PK_CardCustomerFormatCell(List<tblAccessCardExcel> listData, string filename = "", string sheetname = "", string comments = "", string titleSheet = "", string titleTime = "")
        {
            //Nội dung đầu trang
            ...
            listColumn.Add(new SelectListModel { ItemText = "Quyền truy cập", ItemValue = "AccessLevelName" });
            ...
            listColumn.Add(new SelectListModel { ItemText = "Loại thẻ", ItemValue = "CardType" });
        ...
        public ActionResult Create(string key, string cardgroups, string customergroups, string active, string fromdate, string todate, string isCheckByTime = "0")
        {
            ...
            ViewBag.lcustomergroups = GetMenuList();
            ViewBag.llevels = GetListAccessLevel();
            ViewBag.lcardtypes = FunctionHelper.CardChipTypes_CreateUpdate();
        ...
        public ActionResult Create(tblCardSubmit obj, HttpPostedFileBase FileUpload, string key, string cardgroups, string customergroups, string active, string fromdate, string todate, string isCheckByTime = "0", bool SaveAndCountinue = false)
        {
            ...
            var map = new tblCard()
            {
                ...
                ExpireDate = !string.IsNullOrEmpty(obj.DtpDateExpired) ? Convert.ToDateTime(obj.DtpDateExpired) : Convert.ToDateTime("2099/12/31"),
                ...
                CardType = int.Parse(obj.CardType ?? "0"),

                DateCancel = DateTime.Now,
                AccessExpireDate = Convert.ToDateTime("2022/12/31")
            };
            ...
            {
                //Log for hệ thống
                WriteLog.Write(result, GetCurrentUser.GetUser(), map.CardID.ToString(), obj.CardNumber, "tblCard", ConstField.AccessControlCode, ActionConfigO.Create);
        ...
        public ActionResult Update(tblCardSubmit obj, HttpPostedFileBase FileUpload, string key, string cardgroups, string customergroups, string active, string fromdate, string todate, string isCheckByTime = "0", int page = 1)
        {
            ...
            if (map != null)
            {
                ...
                map.AccessLevelID = !string.IsNullOrEmpty(obj.AccessLevelID) ? obj.AccessLevelID : "";
                map.CardType = int.Parse(obj.CardType ?? "0");
            ...
            if (result.isSuccess)
            {
                ...
                //Log for hệ thống
                WriteLog.Write(result, cuuser, oldObj.CardID.ToString(), oldObj.CardNumber, "tblCard", ConstField.AccessControlCode, ActionConfigO.Update);
        ...
        public JsonResult Delete(string id)
        {
            ...
            if (result.isSuccess)
            {
                ...
                WriteLog.Write(result, cuuser, obj.CardID.ToString(), obj.CardNumber, "tblCard", ConstField.AccessControlCode, ActionConfigO.Delete);
        ...
        public List<tblCardGroup> GetListCardGroup()
        {
        ...
        public JsonResult ImportFile()
        {
            ...
            var listcardtype = FunctionHelper.CardChipTypes_CreateUpdate();
            var fileUpload = "";
                            ...
                            if (dt != null && dt.Rows.Count > 0)
                            {
                                foreach (DataRow item in dt.Rows)
                                {
                                    ...
                                    if (!string.IsNullOrWhiteSpace(stt))
                                    {
                                        ...
                                        var role = item["Quyền truy cập"].ToString().Trim();
                                        // var vehiclenames = item["Tên xe"].ToString().Trim();
                                        var _cardtype = listcardtype.Where(p=> p.ItemText == item["Loại thẻ"].ToString().Trim()).FirstOrDefault();
                                        var cardtype = _cardtype != null ? _cardtype.ItemValue : "0";      
                                        ...
                                        //var compartmentId = item["Căn hộ"].ToString().Trim();
                                        //Hoạt động
                                        ...
                                        cardcustom.AccessLevelID = accesslever != null ? accesslever.AccessLevelID.ToString() : "";
                                        ////Gán biển số vào thẻ
                                        ...
                                        //var listVehicleNames = plates.Split(new[] { ';' });
        ...
        private void SetCard_Customer(tblCardSubmit cardsubmit, tblCustomerSubmit customersubmit, bool isNewCard, bool isNewCustomer)
        {
            ...
            if (isNewCard)
            {
                str.AppendLine("INSERT INTO [dbo].[tblCard]([CardNo], [CardNumber], [CustomerID], [CardGroupID], [ImportDate], [ExpireDate], [IsLock], [IsDelete], [Plate1], [VehicleName1], [Plate2], [VehicleName2], [Plate3], [VehicleName3],[AccessLevelID],[CardType])");
                ...
                str.AppendLine(string.Format(", '{0}'", cardsubmit.CardType));
            ...
            else
            {
                ...
                str.AppendLine(string.Format(",[CardType] = '{0}'", cardsubmit.CardType));
        ...
        public JsonResult UpdateCardType(string cardtype,string url = "")
        {
            var result = new MessageReport();
            result.isSuccess = false;
            result.Message = "Có lỗi xảy ra";
            //var lstCard = _tblCardService.GetAll().ToList();
            //foreach (var item in lstCard)
            //{
            //    item.CardType = Convert.ToInt32(cardtype);
            //    result = _tblCardService.Update(item);
            //}
         result =   _tblCardService.UpdateMultiCardType(keyV, cardgroupsV, customeridV, strCGV, fromdateV, todateV, isCheckByTimeV, levelsV, cardtype);
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        private MessageReport ActionProcess(string type, List<string> data, string value = "", string mess = "")
        {
            ...
            try
            {
                ...
                if (result.isSuccess)
                {
                    ...
                    {
                        case "LOCK":
                            WriteLog.Write(result, userCard, listCard, listCard, "tblCard", ConstField.AccessControlCode, ActionConfigO.Lock);
                            break;
                        case "UNLOCK":
                            WriteLog.Write(result, userCard, listCard, listCard, "tblCard", ConstField.AccessControlCode, ActionConfigO.Unlock);
                            break;
                        case "DELETE":
                            WriteLog.Write(result, userCard, listCard, listCard, "tblCard", ConstField.AccessControlCode, ActionConfigO.Delete);
                            break;
                        case "AUTHORIZE":
                            WriteLog.Write(result, userCard, listCard, listCard, "tblCard", ConstField.AccessControlCode, ActionConfigO.Authorize);
                            break;
                        default:
                            break;
                    }
